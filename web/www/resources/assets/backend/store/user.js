...
export default {
    ...
    getters: {
        ...getters,
        types(state) {
            return _.get(state.formData, 'userTypes.results', []) || [];
        },
        services(state) {
            return _.get(state.formData, 'userServices.results', []) || [];
        },
        status(state) {
            return _.get(state.formData, 'userStatus.results', []) || [];
        }
    },
    actions: {
        ...actions,
        async addButtonAction(context, payload) {
            ...
            await context.dispatch('getUserTypes');
            await context.dispatch('getUserServices');
            await context.dispatch('getUserStatus');
